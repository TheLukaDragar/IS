...
def simulate_chromosomes(chromosomes, title="", auto=True, draw=False, custom_colors=[], crossover_data=None, auto_stop=False):
    ...
    while running:
        ...
        if draw:
            for i in range(len(visited)):
                for j in range(len(visited[i])):
                    ...
                    pygame.draw.rect(screen, colorr, (visited[i][j][1] * 20 + (
                        20 - size/2) // 2, visited[i][j][0] * 20 + (20 - size/2) // 2, size/2, size/2))
        ...
        for i in range(len(current_positions)):
            ...
            size = 20 - 2 * i
            pygame.draw.rect(screen, colors[i], (current_positions[i][1] * 20 + (
                20 - size) // 2, current_positions[i][0] * 20 + (20 - size) // 2, size, size))

        # draw crossover data
        if crossover_data:
            if step >= intersection_point_chromosome1_index or step >= intersection_point_chromosome2_index:
                # draw intersection point yellow
                pygame.draw.rect(screen, (255, 255, 0), (intersection_point[1] * 20 + (
                    20 - size) // 2, intersection_point[0] * 20 + (20 - size) // 2, size, size))
            if auto:
