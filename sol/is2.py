...
def mutation_func(offspring, ga_instance):
    ...
    for chromosome_idx in range(offspring.shape[0]):
        ...
        offsprings[chromosome_idx] = mutate_one_chromosome(offspring[chromosome_idx], ga_instance)

    return offsprings

def simulate_crossover():
    #make 100 by 100 plane
    maze_x = 100
    maze_y = 100
...
def crossover_two_chromosomes(chromosome1, chromosome2,sim=False):
    ...
    for i,direction in enumerate(chromosome1):
        ...
        if current_pos not in chromosome1_points:
            ...
            chromosome1_points[current_pos].append(i+1) #append index of move that visits this point
    ...
    intersection_points = list(set(chromosome1_points.keys()) & set(chromosome2_points.keys()))
    ...
    intersection_point_chromosome1_index = choice(chromosome1_points[intersection_point])
    intersection_point_chromosome2_index = choice(chromosome2_points[intersection_point])
    ...
    if(crossover_type=="longest_path_mix"):
        ...
        c1_c2=len(np.concatenate((chromosome1[:intersection_point_chromosome1_index], chromosome2[intersection_point_chromosome2_index:])))
        c1_c2_rtl=len(np.concatenate((chromosome1[:intersection_point_chromosome1_index], chromosome2[:intersection_point_chromosome2_index][::-1])))
        c2_c1=len(np.concatenate((chromosome2[:intersection_point_chromosome2_index], chromosome1[intersection_point_chromosome1_index:])))
        c2_c1_rtl=len(np.concatenate((chromosome2[:intersection_point_chromosome2_index], chromosome1[:intersection_point_chromosome1_index][::-1])))
        if sim:
    ...
    #we choose the random one
        ...
        if glue_combination == "c1_c2":
            ...
            new_chromosome =np.concatenate((chromosome1[:intersection_point_chromosome1_index], chromosome2[:intersection_point_chromosome2_index][::-1]))
        elif glue_combination == "c2_c1":
            new_chromosome=np.concatenate((chromosome2[:intersection_point_chromosome2_index], chromosome1[intersection_point_chromosome1_index:]))
        elif glue_combination == "c2_c1_rtl":
            new_chromosome = np.concatenate((chromosome2[:intersection_point_chromosome2_index], chromosome1[:intersection_point_chromosome1_index][::-1]))
        if sim:
            print("randomly chose glue_combination=",glue_combination,"with length=",len(new_chromosome))
    ...
    if len(new_chromosome) < len(chromosome1):
        ...
        new_chromosome = np.concatenate((new_chromosome, make_valid_chromosome(from_pos=intersection_point, chromosome_length=len(chromosome1) - len(new_chromosome))))
        ...
        if False:
            with open("crossover_edge_cases.txt", "a") as f:
                ...
                f.write(str(intersection_point_chromosome1_index) + " " + str(intersection_point_chromosome2_index) + "\n")
    ...
    if sim:
        ...
        print("Intersection point chromosome 1 index:", intersection_point_chromosome1_index)
        print("Intersection point chromosome 2 index:", intersection_point_chromosome2_index)
        ...
        if len_new < len(chromosome1):
            print("chromosome was too short made it longer by" , len(chromosome1) - len_new,"from",len_new,"to",len(new_chromosome))
        
        print("chose intersection point:", intersection_point,"with rand index in both chromosomes(1,2):", intersection_point_chromosome1_index, intersection_point_chromosome2_index)
        print("and combined chromosomes from and to that index"+"\nadded chromosome1 part:", "".join([str(x) for x in chromosome1[:intersection_point_chromosome1_index]]),"\nadded chromosome2 part:", "".join([str(x) for x in chromosome2[intersection_point_chromosome2_index:]]))
        toprint="".join([str(x) for x in chromosome1[:intersection_point_chromosome1_index]])+" "+"".join([str(x) for x in chromosome2[intersection_point_chromosome2_index:]])
        if len_new < len(chromosome1):
            toprint+=" "+"".join([str(x) for x in new_chromosome])[:len(chromosome1) - len_new]
        ...
        simulate_chromosomes([chromosome1, chromosome2, new_chromosome],title="Crossover",auto=False,draw=True,custom_colors=custom_colors,crossover_data=crossover_data)
...
def crossover_func(parents, offspring_size, ga_instance):
    ...
    for i in range(offspring_size[0]):
        ...
        offspring[i] =  crossover_two_chromosomes(parents[parent1_idx], parents[parent2_idx])
    return offspring
...
def make_valid_chromosome(from_pos=None, chromosome_length=None,smart=False):
    ...
    chromosome_length = chromosome_length if chromosome_length else maze.shape[0] * maze.shape[1]
    ...
    for i in range(chromosome_length):
        ...
        moves=[]
        random_seq = [0, 1, 2, 3] #we make a random sequence of moves to avoid same moves every time
        shuffle(random_seq)
...
def fitness_func(solution, solution_idx):
    ...
    bac_and_forth_moves = 0



    for i in range(len(solution)):

        #we make a move and check if move is valid
        #if move is valid we continue
        #if move is not valid we return distance to end position
        ...
        # elif cnt==2:
        #     #check if we went up down up or down up down or left right left or right left right

            
        #     #ignore if any position contains the end position
        #     if pos != maze_end and prev_pos != maze_end and prev_prev_pos != maze_end:
        #         if pos[0] == prev_pos[0] == prev_prev_pos[0] or pos[1] == prev_pos[1] == prev_prev_pos[1]:
        #             #we went up down up or down up down or left right left or right left right
        #             bac_and_forth_moves += 1
        ...
        if pos == maze_end and pos not in visited:
            found_end = True
            success+=1
            break
        ...
        if pos in points_of_treasures and pos not in visited:
            treasures+=1
            success+=2
    ...
    if found_end and treasures == len(points_of_treasures):
        ...
        return (fitness+ unique_points_visited - repeated_points_visited + path_left*2 ) * treasures
...
def simulate_chromosomes(chromosomes, title="",auto=True,draw=False,custom_colors=[],crossover_data=None):
...
def on_parents(ga_instance, selected_parents):
    ...
    best_chromosomes = ga_instance.best_solutions
    ...
    if(len(best_chromosomes) % 10 == 0):
        ...
        simulate_chromosomes(best_chromosomes, title="Best chromosomes")
...
def start_ga(params):
    ...
    num_parents_matting = params["population_parents_percent"] * params["population_size"]
    num_parents_matting = int(num_parents_matting)
    #make sure is at least 2
    num_parents_matting = max(2, num_parents_matting)

    num_of_elite_to_keep = params["population_keep_elite_percent"] * params["population_size"]
    #int
    num_of_elite_to_keep = int(num_of_elite_to_keep)

    parent_selection_type = params["parent_selection_type"] if "parent_selection_type" in params else "sss" #sss, rws, tournament
    ...
    ga_instance = pygad.GA(num_generations=params["num_generations"],
                            num_parents_mating=num_parents_matting, #number of solutions to be selected as parents in the mating pool.
    ...
    if True ==params["plot"]:
        ...
        plt.ylabel("Fitness")
        plt.title("PyGAD & PyPlot - Iteration vs. Fitness", fontsize=18)
        plt.show()
...
def sim_cross_over_events(chromosome1, chromosome2, new_chromosome, intersection_point, intersection_point_chromosome1_index, intersection_point_chromosome2_index):
        ...
        c3=np.concatenate((chromosome1[:intersection_point_chromosome1_index], chromosome2[intersection_point_chromosome2_index:]))
       
        print("chromosome was too short made it longer by" , len(chromosome1) - len,"from",intersection_point_chromosome1_index)
        ...
        #third color is green
        custom_colors= [(0,0,255),(255,0,0),(0,255,0)]
        simulate_chromosomes([chromosome1, chromosome2, new_chromosome],title="Crossover",auto=False,draw=True,custom_colors=custom_colors)
        #print the chromosomes points
